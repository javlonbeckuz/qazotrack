...
export default function Home() {
  ...
  return <main className={`site-shell ${isNight ? "night" : "day"}`} id="top" lang={language}>
    ...
    <HowItWorks t={t} />
    {user && <QuickAdd counts={counts} t={t} onAdd={(key) => changeCount(key, 1)} />}
 ...
 * rather than a tool.
 */
function QuickAdd({ counts, t, onAdd }: { counts: Counts; t: Copy; onAdd: (key: PrayerKey) => void }) {
  const [open, setOpen] = useState(false);
  const [added, setAdded] = useState<PrayerKey | null>(null);
  ...
  const choose = (key: PrayerKey) => {
    onAdd(key);
    // The sheet stays open: a reader making up several prayers in one sitting
    // should not have to reopen it between each. The tick says it landed.
    setAdded(key);
    window.setTimeout(() => setAdded((current) => (current === key ? null : current)), 1100);
  };
            ...
            {prayerMeta.map((prayer, index) => (
              <li key={prayer.key}>
                <button
                  ...
                  onClick={() => choose(prayer.key)}
                  aria-label={`${t.add} ${t.prayerNames[prayer.key]}`}
                >
                  <span className="arabic">{prayer.arabic}</span>
                  <span className="quick-name">{t.prayerNames[prayer.key]}</span>
                  <span className="row-figure">{counts[prayer.key]}</span>
                  <span className={`quick-tick ${added === prayer.key ? "is-on" : ""}`} aria-hidden="true">

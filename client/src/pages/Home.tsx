...
import { ArrowRight, ChevronRight, Eye, EyeOff, Minus, Moon, Plus, RotateCcw, Settings2, Sun, X } from "lucide-react";
 ...
 *
 * "How it works" used to answer with a single line of notice text, which is not
 * an explanation. This is a dialog rather than a section so that reading it
 * never costs the reader their place on the page.
 */
function HowItWorks({ t, onClose }: { t: Copy; onClose: () => void }) {
  ...
  return (
    <div className="how-backdrop" onClick={onClose}>
      <div
        className="how-panel"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="how-head">
          ...
          <button className="icon-button" onClick={onClose} aria-label={t.howClose}><X size={18} strokeWidth={1.6} /></button>
        </div>
        ...
        <p className="caption how-foot"><span className="trust-dot" />{t.private}</p>
      </div>
    </div>
  );
}

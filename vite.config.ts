...
// Manus workspace:
//
//   vitePluginManusRuntime() inlined a ~366 kB editor runtime into every build,
//   including production. It registers capture-phase `pointerdown` and `click`
//   listeners on `document` so the visual editor can select elements — which
//   swallows real clicks on the app's own buttons. It is the reason the buttons
//   stopped responding outside the Manus preview.
//
//   jsxLocPlugin() annotated JSX with source locations for that same editor. It
//   also pins a peer range of vite@^4||^5 against this project's vite 7, which
//   is what made a plain `npm install` fail with ERESOLVE.
//
...
export default defineConfig({
  ...
  build: {
    // `dist/public` only existed to leave `dist/` free for the server bundle.
    // There is no server now, so the site is the whole of the output.
